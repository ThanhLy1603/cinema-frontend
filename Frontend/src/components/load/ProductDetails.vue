    ...
    <div class="row g-5" v-if="product.id">
      ...
      <div class="col-lg-6">
        <div class="detail-img-wrapper rounded-3 overflow-hidden shadow-sm ratio ratio-1x1">
          <img :src="posterSrc(product.poster)" :alt="product.name"
            class="detail-img img-fluid w-100 h-100 object-fit-cover" @error="handleImageError" />
        </div>
      </div>
      ...
      <div class="col-lg-6">
        <div class="detail-info">
          ...
          <div class="detail-note bg-light p-3 rounded-3 mb-4 border" style="border-color: gainsboro;">
          ...
          <div class="d-flex flex-column gap-3">
            <button @click="buyNow" class="btn btn-success w-100 fw-semibold rounded-pill py-3 shadow-sm">
              Mua Ngay
            </button>
          </div>
        ...
        <div v-for="item in relatedProducts" :key="item.id" class="col-12 col-sm-6 col-md-4 col-lg-3">
          <a @click="goToDetail(item.id)"
            ...
            <div class="ratio ratio-1x1">
              <img :src="posterSrc(item.poster)" :alt="item.name" class="card-img-top object-fit-cover"
...
async function getProduct(id = route.params.id) {
  ...
  try {
    ...
    relatedProducts.value = allRes.data
      .filter(p => p.id !== id)
      .slice(0, 4);
  } catch (error) {
