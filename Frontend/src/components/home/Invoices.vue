    ...
    <div class="min-vh-100 bg-light">
        ...
        <div class="container py-5" style="padding-top: 300px; max-width: 1000px;">
            ...
            <div class="bg-white rounded-4 shadow-lg overflow-hidden">
                ...
                <div class="bg-gradient text-white p-4 p-md-5"
                    style="background: linear-gradient(135deg, #28a745, #20c997);">
                    <div class="row align-items-center g-4">
                        <div class="col-md-3 text-center">
                            <img :src="filmPoster" alt="Poster phim" class="img-fluid rounded shadow-sm"
                                style="max-height: 200px; object-fit: cover;" />
                        </div>
                        <div class="col-md-9">
                            <h2 class="fw-bold mb-2">{{ film.name || 'Tên phim' }}</h2>
                            <div class="d-flex flex-wrap gap-3 fs-5" style="font-size: xx-large;">
                                <p><span><i class="bi bi-calendar3"></i> {{ formatDate(schedule.scheduleDate) }}</span></p>
                                <p><span style="padding-left:10px;"><i class="bi bi-clock"></i> {{ schedule.showTime?.startTime }} </span></p>
                                
                                <p><span><i class="bi bi-geo-alt"></i> {{ schedule.room?.name }} </span></p>
                            </div>
                    ...
                    <div class="row g-5">
                            ...
                            <form @submit.prevent="confirmPayment">
                                <div class="mb-4">
                                    ...
                                    <input v-model="form.fullName" type="text"
                                        class="form-control form-control-lg rounded-pill" placeholder="Nguyễn Văn A"
                                        required />
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input v-model="form.phone" type="tel"
                                        class="form-control form-control-lg rounded-pill" placeholder="0901234567"
                                        required />
                                </div>
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="bg-light rounded-3 p-3 mb-4">
                                <h6 class="fw-bold text-dark mb-3">Vé xem phim ({{ bookingStore.seatCount }} ghế)</h6>
                                <div v-for="seat in bookingStore.bookingInfo.selectedSeats" :key="seat.seatId"
                                    class="d-flex justify-content-between py-2">
                                    <span>{{ seat.position }} <small class="text-muted">({{ seat.seatType || 'Thường'
                                            }}) </small></span>
                                    <strong style="padding-left: 5px;">{{ formatPrice(seat.ticketPrice || seat.price) }}đ</strong>
                                </div>
                            </div>

                            <!-- Combo bắp nước -->
                            <div v-if="bookingStore.productsInCart.length > 0" class="bg-light rounded-3 p-3 mb-4">
                                <h6 class="fw-bold text-dark mb-3">Combo bắp nước</h6>
                                <div v-for="item in bookingStore.productsInCart" :key="item.id"
                                    class="d-flex justify-content-between py-2">
            ...
            <div v-if="showSuccess"
                ...
                <button @click="goHome" class="btn btn-success rounded-pill px-5 mt-3">Về trang chủ</button>
            </div>
        </div>
    </div>
</template>
...
const confirmPayment = async () => {
  try {
    // 1. Lưu thông tin khách vào store (tùy chọn)
    ...
    invoiceForm.value.tickets = bookingStore.bookingInfo.selectedSeats.map(seat => ({
      scheduleId: bookingStore.bookingInfo.schedule, // hỗ trợ cả object và ID
      seatId: seat.seatId,
      ticketPriceId: seat.ticketPriceId, // tên field có thể khác nhau
      price: seat.price ,
      promotionId: null
    }))

    invoiceForm.value.products = bookingStore.productsInCart.map(product => ({
    ...
    // Debug: xem chính xác mình gửi gì
