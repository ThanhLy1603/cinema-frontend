...
export const useBookingStore = defineStore('booking', () => {
  ...
  const cartItemCount = computed(() => {
    ...
    const products = bookingInfo.value.selectedProducts.reduce((sum, item) => sum + (item.quantity || 0), 0)
  ...
  const removeSeat = (seatId) => {
    bookingInfo.value.selectedSeats = bookingInfo.value.selectedSeats.filter(s => s.seatId !== seatId)
  }
  ...
  const removeProduct = (id) => {
    bookingInfo.value.selectedProducts = bookingInfo.value.selectedProducts.filter(p => p.id !== id)
  }
