...
<script setup>
   ...
   async function getCategories() {
      try {
         ...
         categories.value = response.data.filter((c) => !c.isDeleted);
      } catch (err) {
   ...
   async function createCategory() {
      ...
      try {
         const response = await axios.post(API_URL, category.value,
            {
               headers: {
                  Authorization: `Bearer ${token}`,
                  'Content-Type': 'application/json'
   ...
   async function deleteCategory(item) {
      ...
      const confirmed = await showConfirm(`Xác nhận xóa danh mục "${item.name}"?`);

      if (!confirmed) {
         Swal.fire({
            icon: 'info',
            title: 'Đã huỷ xoá danh mục',
            timer: 2000,
