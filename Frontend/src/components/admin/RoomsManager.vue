          ...
          <table class="film-table table-bordered table-hover">
            ...
            <tbody class="text-center">
              <tr v-for="(room, index) in rooms" :key="room.id">
                ...
                <td>
                  <button class="btn btn-sm btn-primary mx-1" @click="handleEdit(room)">Sửa</button>
                  <button class="btn btn-sm btn-danger mx-1" @click="handleDelete(room.id)">Xóa</button>
                </td>
              </tr>
            </tbody>
          </table>
...
async function handleCreateUpdate() {
  ...
  try {
    ...
    alert(showBtnUpdate.value ? 'Cập nhật thành công!' : 'Tạo phòng thành công!');
    await getRooms();
    handleReset();
