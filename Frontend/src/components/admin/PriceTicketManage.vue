<template>
  <div class="container-fluid">
    ...
    <div v-else class="col-md-10 mx-auto mt-3">
      ...
      <div v-if="isEditing" class="col-md-8 mx-auto">
        ...
        <form @submit.prevent="handleUpdate">
          <div class="mb-3">
            <label class="form-label fw-semibold">Loại ngày</label>
            <v-select v-model="form.dayType" :options="dayTypeOptions" label="label" :reduce="d => d.value" required />
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold">Giá vé (VNĐ)</label>
            <input type="number" v-model.number="form.price" class="form-control border-primary" required min="0" step="1000"/>
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold">Ngày bắt đầu áp dụng</label>
            <input type="date" v-model="form.startDate" class="form-control border-primary" required />
          </div>
          <div class="mb-3">
      ...
      <div v-else class="col-12">
        ...
        <div class="table-responsive">
          <table class="table table-bordered align-middle">
            ...
            <tbody>
              <tr v-for="(row, idx) in priceTicketRows" :key="idx" :class="{ 'row-error': row.note }">
                <td>{{ idx + 1 }}</td>
                <td>
                  <v-select v-model="row.filmId" :options="films" label="name" :reduce="f => f.id" @update:modelValue="validateRow(idx)" required />
                </td>
                <td>
                  <v-select v-model="row.seatTypeId" :options="seatTypes" label="name" :reduce="s => s.id" @update:modelValue="validateRow(idx)" required />
                </td>
                <td>
                  <v-select v-model="row.showTimeId" :options="showTimes" label="startTime" :reduce="t => t.id" @update:modelValue="validateRow(idx)" required />
                </td>
                <td>
                  <v-select v-model="row.dayType" :options="dayTypeOptions" label="label" :reduce="d => d.value" @update:modelValue="validateRow(idx)" required />
                </td>
                <td>
                  <input type="number" v-model.number="row.price" class="form-control" @change="validateRow(idx)" required min="0" step="1000"/>
                </td>
                <td>
                  <input type="date" v-model="row.startDate" class="form-control" @change="validateRow(idx)" required />
                </td>
                <td>
                  <input type="date" v-model="row.endDate" class="form-control" @change="validateRow(idx)" />
                </td>
                <td class="text-center note-cell py-3">
                  <div v-if="row.note" class="text-danger fw-500 cursor-pointer" @click="showErrorAlert(row.note)" title="Nhấn để xem chi tiết lỗi">
    ...
    <transition name="fade">
      <div v-if="toast.message" class="toast-custom" :class="toast.type === 'error' ? 'bg-danger' : 'bg-success'">
        {{ toast.message }}
      </div>
    </transition>
  </div>
...
const pageSize = 50
const totalPages = computed(() => Math.ceil(filteredPriceTickets.value.length / pageSize))
const paginatedPriceTickets = computed(() => {
  const start = (currentPage.value - 1) * pageSize
  return filteredPriceTickets.value.slice(start, start + pageSize)
})

// Grid thêm nhiều
const priceTicketRows = ref([
  { filmId: '', seatTypeId: '', showTimeId: '', dayType: '', price: null, startDate: '', endDate: '', note: '' }
])
...
const dayTypeLabel = (type) => dayTypeOptions.find(d => d.value === type)?.label || type || '-'
...
async function loadAllData() {
  try {
    ...
    const [ptRes, fRes, stRes, tRes] = await Promise.all([
      ...
      axios.get(`${API_BASE_URL}/admin/seat-types`, { headers }),
...
function validateRow(idx) {
  ...
  if (!row.filmId || !row.seatTypeId || !row.showTimeId || !row.dayType || !row.price || !row.startDate) return

  if (row.price <= 0) notes.push('Giá vé phải lớn hơn 0')
  if (row.endDate && row.startDate && row.endDate < row.startDate) notes.push('Ngày kết thúc phải ≥ ngày bắt đầu')

  const sameKey = (a, b) => a.filmId === b.filmId && a.seatTypeId === b.seatTypeId && a.showTimeId === b.showTimeId && a.dayType === b.dayType && a.startDate === b.startDate
  ...
  if (priceTickets.value.some(s => sameKey(s, row))) {
    ...
    notes.push(`Trùng cấu hình giá vé từ ngày ${row.startDate}: Phim "${filmName}", Loại ghế "${seatName}"`)
  }
...
function validateAllRows() {
  ...
  priceTicketRows.value.forEach((row, idx) => {
    ...
    if (missing.length > 0 || row.note) {
      hasError = true
      if (missing.length > 0) errorMessages.push(`Dòng ${idx + 1}: Thiếu: ${missing.join(', ')}`)
      if (row.note) errorMessages.push(`Dòng ${idx + 1}: ${row.note}`)
    }
  })

  if (hasError) {
    showErrorAlert("Không thể lưu vì còn lỗi sau:\n\n" + errorMessages.map(m => `• ${m}`).join('\n'))
    return false
  }
  return true
}
...
async function handleUpdate() {
  try {
    ...
    await axios.put(`${API_BASE_URL}/admin/price-tickets/update/${form.value.id}`, payload, { headers })
...
async function handleDelete(id) {
  ...
  if (!ok) return
  try {
    await axios.delete(`${API_BASE_URL}/admin/price-tickets/delete/${id}`, { headers: { Authorization: `Bearer ${token}` } })
...
// Grid helpers
function addRow() {
  priceTicketRows.value.push({ filmId: '', seatTypeId: '', showTimeId: '', dayType: '', price: null, startDate: '', endDate: '', note: '' })
}
...
function removeRow(idx) {
  if (priceTicketRows.value.length > 1) {
    ...
    priceTicketRows.value[0] = { filmId: '', seatTypeId: '', showTimeId: '', dayType: '', price: null, startDate: '', endDate: '', note: '' }
  }
}

function clearAllRows() {
  priceTicketRows.value = [{ filmId: '', seatTypeId: '', showTimeId: '', dayType: '', price: null, startDate: '', endDate: '', note: '' }]
}
...
function resetForm() {
  form.value = { id: null, filmId: '', seatTypeId: '', showTimeId: '', dayType: '', price: null, startDate: '', endDate: '' }
}
...
.container-fluid{padding:10px 15px;position:relative;z-index:1}
.btn-box{display:flex;justify-content:center;gap:10px;margin-bottom:20px}
.switch-btn{background:#b8deb8;border:2px solid #2b2b2b;padding:8px 16px;font-size:16px;border-radius:15px;cursor:pointer;font-weight:bold;transition:.25s}
.switch-btn:hover{transform:translateY(-2px)}
.switch-btn.active{background:#7ee07e;box-shadow:0 0 10px rgba(0,200,0,.3)}
.row-error{background-color:#ffe5e5}
.note-cell{min-width:150px}
.toast-custom{position:fixed;top:20px;right:20px;color:#fff;padding:8px 15px;border-radius:5px;z-index:9999}
.fade-enter-active,.fade-leave-active{transition:opacity .3s}
.fade-enter-from,.fade-leave-to{opacity:0}
.table thead th{position:sticky;top:0;background-color:#d1e7dd;z-index:20!important;box-shadow:0 2px 4px -1px rgba(0,0,0,.1)}
</style>