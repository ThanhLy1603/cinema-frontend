    ...
    <div v-else class="col-md-10 mx-auto mt-3">
      ...
      <div v-if="isEditing" class="col-md-8 mx-auto">
        ...
        <form @submit.prevent="handleUpdate">
          <!-- các trường giống grid nhưng dùng form.filmId, form.seatTypeId ... form.dayType, form.price, form.startDate, form.endDate -->
          <div class="mb-3">
            <label class="form-label fw-semibold">Loại ngày</label>
...
const form = ref({ id: null, filmId: '', seatTypeId: '', showTimeId: '', dayType: '', price: null, startDate: '', endDate: '' });
...
const filteredPriceTickets = computed(() => priceTickets.value.filter((pt) => {
...
function validateRow(idx) {
  ...
  // Kiểm tra trùng với dữ liệu có sẵn
  ...
  priceTicketRows.value.forEach((r, i) => {
    if (i !== idx && sameKey(r, row)) {
      notes.push(`Trùng với dòng ${i + 1} trong bảng`);
    }
  });
...
async function handleCreate() {
  ...
  try {
    ...
    showErrorAlert(err.response?.data?.message || err.message || 'Lỗi khi thêm giá vé');
  }
}
...
async function handleDelete(id) {
  ...
  try {
    const headers = { Authorization: `Bearer ${token}` };
    await axios.delete(`${API_BASE_URL}/admin/price-tickets/delete/${id}`, { headers });
