            ...
            <div class="mb-3" v-else>
               ...
               <div class="email-otp-group">
                  <input
                     ...
                     required
                  />
                  <button type="button" class="btn btn-verify" @click="register">
                     Xác minh OTP
                  </button>
               </div>
            </div>
...
<script setup>
   ...
   async function sendOtp() {
      ...
      if (
         emailStatus.value.includes('đã được sử dụng') ||
         emailStatus.value.includes('không hợp lệ')
      )
   ...
   async function submitForm() {
      ...
      if (
         ...
         return showAlertToast('⚠️ ' + emailStatus.value + '. Vui lòng nhập email khác.');
      if (form.password.length < 7) return showAlertToast('❌ Mật khẩu phải tối thiểu 7 ký tự');
